...
public class Motion extends Application {
    ...
    public void start(Stage stage) {
        ...
        /*Text recText = new Text(data);*/
         notecards.get(frontCardIdx).isFrontProperty().addListener((observable, oldValue, newValue) -> {
            noteCards = makeGroup();
        });
        ...
        root.getChildren().add(noteCards);
    ...
    public void rotateCard(){
        ...
        notecards.get(frontCardIdx).getRect().getTransforms().add(rotate);
        RotateTransition rotateTransition = 
            new RotateTransition(Duration.millis(700), notecards.get(frontCardIdx).getRect());
        ...
        notecards.get(frontCardIdx).setisFront(notecards.get(frontCardIdx).getisFront());//this switches to opposite
    ...
    public void makeAnimation(){
        ...
        PathTransition frontAnim = makeFrontAnimation(frontCard, width, height);
    ...
    public PathTransition makeFrontAnimation(Rectangle frontCard, double width, double height){
    ...
    public Group makeGroup() {
        Group notecardGroup = new Group();
        for(int i = notecards.size()-1; i >= 0; --i){
            
                //Rectangle rec = recList.get(i);
                if(notecards.get(frontCardIdx).getisFront()){
                    notecardGroup.getChildren().add(notecards.get(i).getNoteCardFront());
                }
                else{
                    notecardGroup.getChildren().add(notecards.get(i).getNoteCardBack());
                }
            
            
        }
        
        return notecardGroup;
    }
   ...
   public final boolean getisFront(){return isFront.get();}
    ...
    public StackPane getNoteCardBack(){
        StackPane stack = new StackPane();
        Text text = new Text();
        text.setText(getBackData());
        text.setStyle("fx-width:150px;");
        text.setStyle("fx-text-align:center;");
        text.setStyle("fx-overflow:scroll;");
        stack.getChildren().addAll(rect, text);
